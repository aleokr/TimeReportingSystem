...
namespace ntr_mysqlDatabase.Repository
{
    public class ProjectRepository
    {
        ...
        public Project createProject(ProjectFormData projectFormData)
        {
            User userDb = _dbContext.Users.Where(u => u.Id == projectFormData.UserId).Single();
        ...
        public List<Subproject> getSubprojects(string code)
        {
            Project project = _dbContext.Projects.Where(p => p.Code == code).SingleOrDefault();
            Subproject subproject = new Subproject
            {
            ...
            subprojects.AddRange(_dbContext.Subprojects.Where(s => s.Project.Id == project.Id).ToList());
            return subprojects;
        }

        public List<Project> getProjects()
        {
            return _dbContext.Projects.Where(p => p.Active == true).ToList();
        }
    }
}
