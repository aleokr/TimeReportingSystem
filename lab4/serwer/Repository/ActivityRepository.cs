...
namespace ntr_mysqlDatabase.Repository
{
    public class ActivityRepository
    {
        ...
        public Activity addActivity(ActivityForm activityFormData, int userId)
        {
            User user = _dbContext.Users.Where(u => u.Id == userId).Single();
            Project project = _dbContext.Projects.Where(p => p.Code == activityFormData.Code).Single();

            Activity activity = new Activity
            {
                Date = activityFormData.Date,
                Code = activityFormData.Code,
                Subcode = activityFormData.Subcode,
        ...
        public void updateActivity(UpdateActivityFormData acceptActivityFormData)
        {
            ...
            if (activity != null)
            {
                ...
                activity.Accepted = acceptActivityFormData.Submit;
                activity.Description = acceptActivityFormData.Description;
        ...
        internal Activity GetActivityById(int activityId)
        {
            Activity activity = _dbContext.Activities.Where(a => a.Id == activityId).SingleOrDefault();

            User user = _dbContext.Users.Where(u => u.Id == activity.UserId).Single();
            Project project = _dbContext.Projects.Where(p => p.Code == activity.Code).Single();
